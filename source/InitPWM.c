...
void InitPWM(){
	...
	TPM1->SC |= TPM_SC_PS(0);	//	clock divider = 64; clock=655360Hz
	TPM1->MOD = 1667;		//	 MODULO=25 - f_pwm=25kHz (we need 25kHz to eliminate irritating sounds the car may make)

	//right engine
	TPM1->CONTROLS[1].CnSC = TPM_CnSC_MSB_MASK|TPM_CnSC_ELSB_MASK;	//	TPM1 - Edge-aligned PWM Low-true pulses (set Output on match, clear Output on reload)
	TPM1->CONTROLS[1].CnV = 0x0000;					// fill factor = 0

	//left engine
	TPM1->CONTROLS[0].CnSC = TPM_CnSC_MSB_MASK|TPM_CnSC_ELSB_MASK;	//	TPM1 - Edge-aligned PWM Low-true pulses (set Output on match, clear Output on reload)
